...
MAX_SHORT_POSITION_SIZE = 0.045
MAX_LONG_POSITION_SIZE = 0.045
...
def make_factors():
    ...
    class AssetsTurnoverChange(CustomFactor):
        ...
        def compute(self, today, assets, out, assets_turnover):
            out[:] = preprocess(assets_turnover[-1] > assets_turnover[0]).astype(int) 
...
def make_pipeline():
   ...
   # ===============================================    
    ...
    #Screen Value
    universe1 = value.bottom(3*(NUM_LONG_POSITIONS + NUM_SHORT_POSITIONS), mask = (QTradableStocksUS() & market_cap))
    universe2 = value.top(3*(NUM_LONG_POSITIONS + NUM_SHORT_POSITIONS), mask = (QTradableStocksUS() & market_cap))
